...
if ! grep -Eq 'RESULT ceFired=2 elementFired=4 childDisconnected=1 ceHidden=(true|false) elementHidden=(true|false) childHidden=(true|false)' "$log"; then
  echo "FAIL  expected all seven custom-element callbacks to read document.hidden"
  failures=$((failures + 1))
fi
if [ "$counts" != "8 1 1" ]; then
  echo "FAIL  expected hidden/bgColor-get/bgColor-set counts '8 1 1', got '${counts:-none}'"
  failures=$((failures + 1))
fi
